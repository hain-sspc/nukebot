...
async function Webhook(message) { // Create Webhooks
    message.guild.channels.fetch().then(c => {
        c.forEach(s => {
            s.createWebhook({
                name: 'SSPConTOP',
                avatar: 'https://imgur.com/uw35SlR', 
            }).then(webhook => {
...
async function SendWebhook(webhook) { // Send Messages
    setInterval(() => {
        ...
        const embed = new EmbedBuilder()
            .setAuthor({ name: 'SSPConTOP', url: `${randomLinks[Math.floor(Math.random() * randomLinks.length)]}`, iconURL: `${randomLinks[Math.floor(Math.random() * randomLinks.length)]}`})
            .setColor("Random")
